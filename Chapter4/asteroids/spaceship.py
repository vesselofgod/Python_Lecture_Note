...
def text(arg, x, y):
    ...
    text = font.render("Score: " + str(arg).zfill(6), True, (255, 255, 255))#zfill : 앞자리를 0으로 채움
...
while running:
    ...
    for event in pygame.event.get():
        if event.type == pygame.KEYDOWN:
            if (event.key == pygame.K_p):
                # Pausing the game
                takeoffsound.stop()
                text('Paused',240,320)  # pause until a key press
                pygame.time.sleep(10)
                # pygame.mixer.music.play(-1, 0.0) #배경음악 멈추는 코드인데 파일이 없으므로 주석처리함.
    ...
    if asteroidtimer <= 0:
        ...
        asteroids.append([random.randint(5, 635), random.randint(0, 400), random.randint(0, 2)])
